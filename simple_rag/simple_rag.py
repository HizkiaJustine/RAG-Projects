...
class EmbeddingModel:
    def __init__(self, model_type="gemini"):
        ...
        if model_type == "gemini":
            ...
            self.embedding_fn = embedding_functions.GoogleGeminiEmbeddingFunction(
...
def select_models():
    ...
    while True:
        ...
        if choice in ["1", "2"]:
            llm_type = "gemini" if choice == "1" else "ollama"
            break
        print("Please enter either 1 or 2")

    # Select Embedding Model
    print("\nSelect Embedding Model:")
    print("1. Gemini Embeddings")
    print("2. Chroma Default")
    print("3. Nomic Embed Text (Ollama)")
    while True:
        choice = input("Enter choice (1, 2, or 3): ").strip()
        if choice in ["1", "2", "3"]:
            embedding_type = {"1": "gemini", "2": "chroma", "3": "nomic"}[choice]
            break
...
def rag_pipeline(query, collection, llm_model, top_k=2):
    ...
    system_instruction = (
        "You are a helpful assistant who can answer questions about space "
        "but only answers questions that are directly related to the sources/documents given."
    )
